...
class MusicActivity : AppCompatActivity() {
    ...
    private lateinit var playerBar: View   // << mini-player

    private val tracks = mutableListOf<Track>()
    private lateinit var adapter: TracksAdapter
    private var currentIndex = -1
    private var lastPlayingUri: Uri? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        adapter = TracksAdapter(
            ...
            onRowDelete = { pos ->
                if (pos in tracks.indices) deleteTrack(tracks[pos].uri, pos)
            }
        )
        ...
        refreshList() // también recoloca el FAB
    }
    ...
    // ==== mover FAB según visibilidad del mini-player ====
    private fun Int.dp(): Int = (this * resources.displayMetrics.density).toInt()

    private fun bumpFabAbovePlayerBar(showing: Boolean) {
        val base = 16.dp()
        val bottom = if (showing) playerBar.height + base else base
        val lp = fabAdd.layoutParams
    ...
    private fun refreshList() {
        ...
        playerBar.visibility = if (isEmpty) View.GONE else View.VISIBLE

        if (isEmpty) {
            tvNowPlaying.text = getString(R.string.music_now_playing)
            updatePlayPauseIcon(false)
            currentIndex = -1
            lastPlayingUri = null
        }

        // Recolocar FAB después de medir la altura real del playerBar
        playerBar.post { bumpFabAbovePlayerBar(!isEmpty) }
    ...
    // ==== Eliminar pista ====
    private fun deleteTrack(uri: Uri, posHint: Int? = null) {
        // 1) SharedPreferences
        val key = "music_uris"
        val prefs = getPrefs()
        val current = (prefs.getStringSet(key, emptySet()) ?: emptySet()).toMutableSet()
        if (current.remove(uri.toString())) {
            prefs.edit().putStringSet(key, current).apply()
        }

        // 2) Lista/adapter
        val idx = posHint?.takeIf { it in tracks.indices } ?: tracks.indexOfFirst { it.uri == uri }
        if (idx >= 0) {
            tracks.removeAt(idx)
        ...
        // 3) Si era la que sonaba, detener
        ...
        // 4) Vacío/visibilidad + recolocar FAB
        refreshList()
    }
    ...
    private class TracksAdapter(
        ...
        private val onRowDelete: (Int) -> Unit
        ...
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): TrackVH {
            ...
            return TrackVH(v, onRowClick, onRowPlay, onRowDelete)
        }
    ...
    private class TrackVH(
        ...
        val onRowDelete: (Int) -> Unit
    ) : RecyclerView.ViewHolder(itemView) {
        private val tvName = itemView.findViewById<TextView>(R.id.tvName)
        private val btnPlay = itemView.findViewById<ImageButton>(R.id.btnRowPlay)
        private val btnDelete = itemView.findViewById<ImageButton>(R.id.btnRowDelete)
        ...
        fun bind(t: Track, isCurrent: Boolean) {
            ...
            btnDelete.setOnClickListener { onRowDelete(bindingAdapterPosition) }
        }
    }
}
